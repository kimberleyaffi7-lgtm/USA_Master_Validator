// Paid-only USA Validator premium validation engine.
// This file is intentionally NOT in the Vite public bundle.
// Render serves it only after a server-side Supabase entitlement check.
const AREA_CODES = {
...
export function runPremiumChecks({ phone, ssn, dl, routing, bank, state, phoneAreaCheck, identityCheck, financeCheck }) {
  const issues = [];
  let penalty = 0;

  if (phoneAreaCheck && phone) {
    let p = String(phone).replace(/\D/g, "");
    if (p.length === 11 && p.startsWith("1")) p = p.slice(1);
    if (p.length === 10 && state) {
      const expected = AREA_CODES[p.slice(0, 3)];
      if (expected && expected !== state) {
        issues.push("Area code mismatch");
        penalty += 10;
      }
    }
  }
  ...
  if (identityCheck) {
    if (ssn && !ssnFormatValid(ssn)) {
      issues.push("Invalid SSN format");
      penalty += 50;
    }
    const cleanDl = String(dl ?? "").replace(/[^a-z0-9]/gi, "");
    if (dl && (cleanDl.length < 5 || cleanDl.length > 20)) {
      issues.push("Invalid DL format");
      penalty += 20;
    }
  }

  if (financeCheck) {
    if (routing && !routingValid(routing)) {
      issues.push("ABA checksum failed");
      penalty += 50;
    }
    const cleanBank = String(bank ?? "").replace(/\D/g, "");
    if (bank && (cleanBank.length < 4 || cleanBank.length > 17)) {
      issues.push("Invalid bank account length");
      penalty += 20;
    }
  }
